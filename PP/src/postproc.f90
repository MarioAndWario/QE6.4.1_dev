...
CONTAINS
  ...
  SUBROUTINE extract (plot_files,plot_num)
    ...
    ! ======
    character(LEN=50) :: emin_inner_ev_string, emax_inner_ev_string

    INTEGER :: inpunit=2333
    ...
    IF ( ionode )  THEN
       ...
       READ (inpunit, inputpp, iostat=ios)
       !
    ...
    ! ======
    IF (plot_num .ne. 100) then
       IF ( plot_num < 0 .or. plot_num > 22) then
          CALL errore ('postproc', 'Wrong plot_num', abs (plot_num) )
       ENDIF
    ENDIF
    ...
    needwf=(plot_num==3).or.(plot_num==4).or.(plot_num==5).or.(plot_num==7).or. &
         (plot_num==8).or.(plot_num==10) .or. (plot_num == 100)
    ...
    ! ======
    IF ((plot_num == 10) .or. (plot_num == 100)) THEN
    ...
    IF (plot_num == 100) THEN

       ! ======
       ! Decide emin_inner, emax_inner
       ! ------
       emin_inner = emin
       emax_inner = emin_inner + delta_e

       ! write(*,*) "emin_inner = ", emin_inner, " emax_inner = ", emax_inner, " delta_e = ", delta_e
       do while (emax_inner <= emax+0.000001)

          if (ionode) then

             write(emin_inner_ev_string,'(f10.4)') emin_inner*rytoev
             write(emax_inner_ev_string,'(f10.4)') emax_inner*rytoev

             filplot="ldos."//trim(adjustl(emin_inner_ev_string))//"_"//trim(adjustl(emax_inner_ev_string))//"eV.dat"
          ...
          CALL punch_plot (filplot, plot_num, sample_bias, z, dz, &
               emin_inner, emax_inner, kpoint, kband, spin_component, lsign)
          ...
          if (ionode) then
             ! ======
             ! [IMPORTANT]
             ! we need to rewind the input file, or chdens() won't get input parameters
             REWIND (inpunit)
          endif

          emin_inner = emin_inner + delta_e
          emax_inner = emax_inner + delta_e
